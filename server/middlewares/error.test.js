...
describe("Given an errorHandler middleware,", () => {
  describe("When it gets a request and an error and no error code", () => {
    test("Then it should send a response with a 'Not working' error and a status code of 500", () => {
      const res = mockResponse();
      const error = { error: "Not working" };
      const req = {};
      const next = () => {};
  ...
  describe("When it gets a request and a Validation error", () => {
    test("Then it should send a response with a 'Bad request' message and a status code of 400", () => {
      const res = mockResponse();
      ...
      expect(res.json).toHaveBeenCalledWith({ error: "Bad request :( LOL" });
    });
  });
});
